...
class GravamePayload(TypedDict):
    tipo: str
    placa: str
    homolog: bool


class GravameResponse(TypedDict, total=False):
    status: bool
    message: str
    dados: dict
...
class GravameService:
    ...
    def consultar_placa(self, placa: str, homolog: bool = False) -> GravameResponse:
        payload: GravamePayload = {
            "tipo": "gravame",
            "placa": placa,
            "homolog": homolog,
        }
        ...
        try:
            ...
            raise APIBrasilTimeoutError(
                "Timeout ao consultar Gravame", status_code=408
            ) from exc
    ...
    @staticmethod
    def _handle_response(response: requests.Response) -> GravameResponse:
        try:
            body = response.json()
        except ValueError:
            body = {}
