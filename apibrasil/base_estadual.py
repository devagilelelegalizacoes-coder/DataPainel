...
class BaseEstadualPayload(TypedDict):
    tipo: str
    placa: str
    homolog: bool


class BaseEstadualResponse(TypedDict, total=False):
    status: bool
    message: str
    dados: dict
...
class BaseEstadualService:
    ...
    def consultar_placa(self, placa: str, homolog: bool = False) -> BaseEstadualResponse:
        payload: BaseEstadualPayload = {
            "tipo": "estadual",
            "placa": placa,
            "homolog": homolog,
        }
        ...
        try:
            ...
            raise APIBrasilTimeoutError(
                "Timeout ao consultar Base Estadual", status_code=408
            ) from exc
    ...
    @staticmethod
    def _handle_response(response: requests.Response) -> BaseEstadualResponse:
        try:
            body = response.json()
        except ValueError:
            body = {}
