...
class DebitosV4Payload(TypedDict):
    tipo: str
    placa: str
    homolog: bool


class DebitosV4Response(TypedDict, total=False):
    status: bool
    message: str
    dados: dict


class DebitosV4Service:
    ...
    def consultar_placa(self, placa: str, homolog: bool = False) -> DebitosV4Response:
        payload: DebitosV4Payload = {
            "tipo": "debitos-v4",
            "placa": placa,
            "homolog": homolog,
        }
        ...
        try:
            ...
            raise APIBrasilTimeoutError(
                "Timeout ao consultar Débitos V4", status_code=408
            ) from exc
    ...
    @staticmethod
    def _handle_response(response: requests.Response) -> DebitosV4Response:
        try:
            body = response.json()
        except ValueError:
            body = {}
