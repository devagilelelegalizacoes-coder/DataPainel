...
class AgregadosPropriaPayload(TypedDict):
    tipo: str
    placa: str
    homolog: bool


class AgregadosPropriaResponse(TypedDict, total=False):
    status: bool
    message: str
    dados: dict


class AgregadosPropriaService:
    ...
    def consultar_placa(self, placa: str, homolog: bool = False) -> AgregadosPropriaResponse:
        payload: AgregadosPropriaPayload = {
        ...
        try:
            ...
            raise APIBrasilTimeoutError(
                "Timeout ao consultar Agregados Própria", status_code=408
            ) from exc
    ...
    @staticmethod
    def _handle_response(response: requests.Response) -> AgregadosPropriaResponse:
        try:
            body = response.json()
        except ValueError:
            body = {}
