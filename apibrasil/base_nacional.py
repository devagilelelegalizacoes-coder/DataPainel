...
class BaseNacionalPayload(TypedDict):
    tipo: str
    placa: str
    homolog: bool


class BaseNacionalResponse(TypedDict, total=False):
    status: bool
    message: str
    dados: dict


class BaseNacionalService:
    ...
    def consultar_placa(self, placa: str, homolog: bool = False) -> BaseNacionalResponse:
        payload: BaseNacionalPayload = {
            "tipo": "nacional",
            "placa": placa,
            "homolog": homolog,
        }
        ...
        try:
            ...
            raise APIBrasilTimeoutError(
                "Timeout ao consultar Base Nacional", status_code=408
            ) from exc
    ...
    @staticmethod
    def _handle_response(response: requests.Response) -> BaseNacionalResponse:
        try:
            body = response.json()
        except ValueError:
            body = {}
